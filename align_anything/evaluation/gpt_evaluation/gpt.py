...
from align_anything.evaluation.gpt_evaluation.answer_generation_prompt import system_prompt,  user_prompt, image_prompt, image_user_prompt
...
def bean_dalle_api(
    ...
    messages = [
        {'role': 'system', 'content': image_prompt},
        {
            'role': 'user',
            "content": [
                {"type": "text", "text": image_user_prompt.format(question=user_content)},
                {
                    "type": "image_url",
                    "image_url": {
                        "url": image_url,
...
def image_api(
    ...
    """API"""
    server = bean_dalle_api
    api_interaction_count = 0
    ray.init()
    contents = list(enumerate(zip(user_contents, image_urls)))
    bar = tqdm(total=len(user_contents))
...
def extract_instruction_response(input_string):
    instruction_pattern = r"\[\[IMAGEWORD\]\](.*?)\[\[RESPONSE\]\]"
    response_pattern = r"\[\[RESPONSE\]\](.*)"

    instruction_match = re.search(instruction_pattern, input_string, re.DOTALL)
    response_match = re.search(response_pattern, input_string, re.DOTALL)

    instruction_content = instruction_match.group(1).strip() if instruction_match else None
    response_content = response_match.group(1).strip() if response_match else None

    return instruction_content, response_content

def extract_first_url(text):
    # 定义正则表达式模式，仅匹配以 .webp 结尾的URL
    url_pattern = r'http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\\(\\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+\.webp'
    # 使用正则表达式搜索第一个匹配的URL
...
def answer_generation():
    with open('/home/yangyaodong/projects/jiayi/data/30000_question_generation.json', 'r') as file:
        data = json.load(file)[0:10000]
        
    processed_image_dataset = [item['image_url'] for item in data]
    user_prompts = [user_prompt.format(question=item['question']) for item in data]
    
    def post_process(response: str):
        return response
    
    final_data = []
    system_prompts = [system_prompt]*len(processed_image_dataset)
    
    results = api(system_prompts, user_prompts, processed_image_dataset, post_process=post_process)
    
    instructions = []
    responses = []
    for i in range(len(results)):
        instruction, response = extract_instruction_response(results[i])
        instructions.append(instruction)
        responses.append(response)
    image_results = image_api(instructions, processed_image_dataset)
    
    for i in range(len(results)):
        final_item = data[i]
        instruction, response = extract_instruction_response(results[i])
        final_item['image_prompt'] = instruction
        final_item['response'] = response
        final_item['output_image_url'] = extract_first_url(image_results[i])
        final_data.append(final_item)
    
    ouput_dir = '/home/yangyaodong/projects/jiayi/data/10000_full.json'
